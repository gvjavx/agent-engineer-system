...
- **Edit gambar**: kirim gambar dengan caption yang diawali kata kerja ubah — "ubah jadi malam hari", "jadikan hitam putih", "ganti background jadi pantai". Backend edit: **Hugging Face Inference** (`instruct-pix2pix`) kalau `HF_API_TOKEN` diisi (token gratis, tanpa kartu — edit di gambar asli, komposisi kejaga; free tier rate-limited + cold-start, retry sekali). Kalau nggak diisi atau gagal → img2img Cloudflare (`CLOUDFLARE_EDIT_MODEL`, sering di-gate per-akun). Kalau dua-duanya gagal → fallback terakhir: gambarnya dideskripsikan lalu dibikin ulang dari deskripsi + ubahanmu (komposisi bisa beda). Caption yang isinya kerjaan koding ("perbaiki tampilan sesuai screenshot ini") tetap lewat jalur vision → task seperti biasa.
