...
- **Edit gambar**: kirim gambar dengan caption yang diawali kata kerja ubah — "ubah jadi malam hari", "jadikan hitam putih", "ganti background jadi pantai". Ditarik ke img2img Cloudflare (`dreamshaper-8-lcm` default; override lewat `CLOUDFLARE_EDIT_MODEL`), komposisi asli dipertahankan (`strength` ~0.6). Kalau modelnya gak bisa diakses akun Cloudflare-mu (403), otomatis fallback: gambarnya dideskripsikan dulu terus dibikin ulang dari deskripsi + ubahanmu (komposisi bisa beda). Caption yang isinya kerjaan koding ("perbaiki tampilan sesuai screenshot ini") tetap lewat jalur vision → task seperti biasa.
