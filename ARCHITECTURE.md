...
- **Data usang** ditangani dua lapis: (1) `isVolatile` (`agent/chatKb.ts`) — pas rekam, kalau pertanyaan/jawaban mengandung penanda time-sensitive (harga/kurs/cuaca/"sekarang"/"terbaru"/jabatan/tahun 20xx/...) baris disimpan sebagai `chat_volatile` yang nggak pernah dicocokin; bias ke over-flag (rugi paling banter satu panggilan model ekstra). (2) TTL `CHAT_KB_MAX_AGE_DAYS` (default 90) — baris lebih tua dari itu di-skip pas lookup, pertanyaannya balik ke model, jawaban tersimpan ditimpa (insert `chat_model` ngehapus baris `norm_question` yang sama dulu).
- **Batasnya**: cuma bantu pertanyaan yang beneran diulang (wording mirip). Pertanyaan baru tetap ke Gemini. Jawaban tersimpan = rekaman jawaban Gemini dulu (masih bisa salah kalau Gemini-nya yang salah — belum ada mekanisme koreksi).
