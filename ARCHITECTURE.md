...
Tiap provider (`providers/gemini.ts`, `providers/openAiCompatible.ts`) implement interface `Provider` yang sama (`chat(messages, tools, signal)`, opsional `describeImage(...)`, `transcribeAudio(...)`, `synthesizeSpeech(...)`, `generateImage(...)`) — kode di atasnya (loop, classifier, chat assistant) nggak pernah tahu lagi vendor mana yang lagi dipakai. Cuma Gemini yang implement yang opsional-opsional itu, dan tiap masing-masing digate model tersendiri (`GEMINI_TTS_MODEL`, `GEMINI_IMAGE_MODEL`); `agent/imageDescription.ts`, `agent/audioTranscription.ts`, `agent/imageGeneration.ts` nyisir daftar provider dan nge-skip yang nggak punya.
...
7. Tiap fase jalan lewat `agent/loop.ts` (`runAgentLoop`) — loop tool-calling: panggil provider dengan daftar tool, kalau responsnya `tool_calls` eksekusi satu-satu lalu kasih hasilnya balik ke model, ulang sampai model kasih jawaban teks (itu tandanya selesai) atau `maxTurns` habis (40 buat task biasa, 25 per fase pipeline, 40 QA, 12 buat `tanya:`). Kalau provider error, otomatis pindah ke provider/key berikutnya dalam giliran yang sama. **Loop guard**: tiap tool call di-hash (`nama: briefToolDescription`); yang sama muncul kelipatan 3x → sisipin pesan `role:"user"` `[sistem]` nyuruh ganti pendekatan / simpulin (role `user` bukan `system` karena adapter Gemini buang system message di tengah percakapan). Pas sisa ~6 langkah, satu peringatan budget disisipin juga.
...
- **Guard diff kegedean** (`agent/diffGuard.ts`, `DIFF_GUARD_ENABLED`, default on): sebelum `git commit`, `git diff --cached --numstat` di-jumlahin; kalau > `DIFF_GUARD_MAX_FILES` (60) file atau > `DIFF_GUARD_MAX_LINES` (1500) baris → pause, tanya ya/tidak lewat `onDangerousBash` yang udah ada. Sekali per task: approve → commit-commit sisanya skip; deny → commit ini diblok dan diff yang masih kegedean tetep diblok tanpa nanya ulang sampai dikecilin.
